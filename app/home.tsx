...
export default function HomeScreen() {
  ...
  const load = useCallback(async (soft = false) => {
    ...
    try {
      ...
      if (["system_admin", "manager", "viewer"].includes(stored.role)) {
  ...
  const startShift = async () => {
    ...
    try {
      ...
      Alert.alert("لم يبدأ الشفت", e instanceof Error ? e.message : "تعذر بدء الشفت");
    ...
    <SafeAreaView style={styles.page}>
      <ScrollView contentContainerStyle={styles.content} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={() => { setRefreshing(true); load(true); }} />}>
        <View style={styles.header}>
          <View style={{ flex: 1 }}><Text style={styles.hello}>مرحبًا بك</Text><Text style={styles.name}>{profile.full_name}</Text><Text style={styles.muted}>يوم العمل: {businessDate()}</Text></View>
          <TouchableOpacity onPress={signOut}><Text style={styles.logout}>خروج</Text></TouchableOpacity>
        </View>
        {!!error && <View style={styles.errorBox}><Text style={styles.errorText}>{error}</Text></View>}
        <View style={styles.targetCard}>
          <Text style={styles.targetLabel}>التارجت الشهري</Text><Text style={styles.targetValue}>{monthlyTarget.toLocaleString()}</Text><Text style={styles.targetPoints}>نقطة</Text>
          ...
          <View style={styles.targetRow}>
            ...
            <View><Text style={styles.smallLabel}>نسبة التحقيق</Text><Text style={styles.smallValue}>{achievement.toFixed(1)}%</Text></View>
          </View>
        </View>
        <Text style={styles.sectionTitle}>ملخص اليوم</Text>
        <View style={styles.grid}><Stat label="نقاط اليوم" value={String(daily.total_points ?? 0)} /><Stat label="إجمالي الشرائح" value={String(daily.total_units ?? 0)} /></View>
        <View style={styles.grid}><Stat label="حالة المبيعات" value={salesLabel(daily.sales_status)} /><Stat label="حالة الشفت" value={shiftLabel(shiftStatus)} /></View>
        ...
        {shiftStatus === "not_started" && <><TouchableOpacity disabled={busy} style={styles.primaryButton} onPress={startShift}><Text style={styles.primaryButtonText}>{busy ? "جاري تحديد الموقع…" : "بداية الشفت"}</Text></TouchableOpacity><TouchableOpacity disabled={busy} style={styles.dangerGhost} onPress={markAbsent}><Text style={styles.dangerText}>رفع غياب اليوم</Text></TouchableOpacity></>}
        {shiftStatus === "open" && <TouchableOpacity disabled={busy} style={styles.primaryButton} onPress={endShift}><Text style={styles.primaryButtonText}>{busy ? "جاري الحفظ…" : "إنهاء الشفت"}</Text></TouchableOpacity>}
        {shiftStatus === "ended" && <TouchableOpacity style={styles.primaryButton} onPress={() => router.push("/sales")}><Text style={styles.primaryButtonText}>تسجيل / تعديل المبيعات</Text></TouchableOpacity>}
...
function PackageCard({ code, qty }: { code: keyof typeof PACKAGE_POINTS; qty: number }) { return <View style={styles.packageCard}><Text style={styles.packageName}>باقة {code}</Text><Text style={styles.packageQty}>{qty} شريحة</Text><Text style={styles.packageValue}>{PACKAGE_POINTS[code]} نقاط / شريحة</Text></View>; }
...
const styles = StyleSheet.create({
  page: { flex: 1, backgroundColor: "#F4F7FB" }, center: { alignItems: "center", justifyContent: "center" }, content: { paddingHorizontal: 20, paddingTop: 20, paddingBottom: 50 },
  header: { flexDirection: "row-reverse", justifyContent: "space-between", alignItems: "center", marginBottom: 22, gap: 12 }, hello: { textAlign: "right", color: "#8B96A5", fontSize: 14 }, name: { textAlign: "right", color: "#17233C", fontSize: 21, fontWeight: "900", marginTop: 4 }, muted: { color: "#8792A1", marginTop: 8, textAlign: "center" }, logout: { color: "#B42318", fontWeight: "900", padding: 10 },
  errorBox: { backgroundColor: "#FEF3F2", borderRadius: 14, padding: 12, marginBottom: 14 }, errorText: { color: "#B42318", textAlign: "right", fontWeight: "700" },
  targetCard: { backgroundColor: "#17233C", borderRadius: 28, padding: 22, marginBottom: 28 }, targetLabel: { color: "#BCC5D2", textAlign: "center", fontSize: 15 }, targetValue: { color: "#FFFFFF", textAlign: "center", fontSize: 44, fontWeight: "900", marginTop: 5 }, targetPoints: { color: "#65D9B5", textAlign: "center", fontWeight: "800", fontSize: 16 }, separator: { height: 1, backgroundColor: "#33405A", marginVertical: 18 }, targetRow: { flexDirection: "row-reverse", justifyContent: "space-around", gap: 8 }, smallLabel: { color: "#AEB9C9", textAlign: "center", fontSize: 11 }, smallValue: { color: "#FFFFFF", textAlign: "center", fontSize: 17, fontWeight: "900", marginTop: 5 },
  sectionTitle: { textAlign: "right", fontSize: 19, fontWeight: "900", color: "#17233C", marginBottom: 13, marginTop: 4 }, grid: { flexDirection: "row-reverse", gap: 12, marginBottom: 12 }, statCard: { flex: 1, backgroundColor: "#FFFFFF", borderRadius: 20, padding: 17 }, statLabel: { textAlign: "right", color: "#8792A1", fontSize: 12 }, statValue: { textAlign: "right", color: "#17233C", fontSize: 18, fontWeight: "900", marginTop: 7 },
  packageGrid: { flexDirection: "row-reverse", flexWrap: "wrap", gap: 12, marginBottom: 25 }, packageCard: { width: "48%", backgroundColor: "#FFFFFF", borderRadius: 19, padding: 16 }, packageName: { textAlign: "right", color: "#17233C", fontSize: 17, fontWeight: "900" }, packageQty: { textAlign: "right", color: "#17233C", fontSize: 22, fontWeight: "900", marginTop: 7 }, packageValue: { textAlign: "right", marginTop: 5, color: "#16A77A", fontWeight: "800", fontSize: 12 },
  primaryButton: { backgroundColor: "#16A77A", borderRadius: 18, paddingVertical: 17, marginBottom: 12 }, primaryButtonText: { textAlign: "center", color: "#FFFFFF", fontWeight: "900", fontSize: 17 }, dangerGhost: { borderWidth: 1, borderColor: "#FDA29B", borderRadius: 18, paddingVertical: 15 }, dangerText: { color: "#B42318", textAlign: "center", fontWeight: "900" }, absentBox: { backgroundColor: "#FEF3F2", borderRadius: 18, padding: 16 }, absentText: { color: "#B42318", textAlign: "center", fontWeight: "900" },
});